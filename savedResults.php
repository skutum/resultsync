...
<html>
<head>
<title>Result Sync | Saved Results</title>
...
$(document).ready( function() {
   ...
   $('.delete').click( function() {
        ...
        var url = 'http://resultsync.com/targets/savedResultsDelete.php?id=' + id;
        var button = $(this);
...
<body>
   <div id="suggestPost"><a href="http://resultsync.com/suggestions.php?sent=0" ></a></div>
    <div id="page-container">
        ...
        <div id="content">
            <?php if(isLoggedIn()){?>
            <h1>Saved Results </h1><h2> - Here you can see any results that you have saved.</h2><br><br>
            
            <?php
                 // find out how many rows are in the table 
                $sql = "SELECT COUNT(*) FROM savedResults WHERE userId=" . getuserid();
                ...
                $savequery = "SELECT saveId, userId, resultId FROM savedResults WHERE userId=".getuserid()." LIMIT $offset, $rowsperpage";
                $saveresult= mysql_query($savequery);
                while($saverow = mysql_fetch_array($saveresult)) {
            
                $query = "SELECT id, title, imageOne, numberOfViews, rating FROM  `results` WHERE id=" . $saverow['resultId'] . "  ORDER BY  `dateCreated` DESC ";
                ...
                while($row = mysql_fetch_array($result)) { ?>
                    ...
                    <div id="result" class="result">
                            ...
                            <input type="button" class="delete" data-id="<?php echo $saverow['saveId']; ?>" value="Delete"></input>    
                    </div>
            <?php } }?>
